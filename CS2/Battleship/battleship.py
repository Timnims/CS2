...
class Battle_Ship(Fl_Window):
	def __init__(self, x, y, w, h, label=f'Battleship [{sys.argv[1].title()}]'):
	...
	def place_ships(self, w):  # When you get to place your ships
		...
		if ind not in self.myship_list:
			...
			if self.ships == 5: # Check if all ships are placed
				self.am_ready = True
				self.ready()
				return
...
size = 600
app = Battle_Ship(0, 650, 1125, size)
Fl.scheme('plastic')
app.show()
Fl.run()
